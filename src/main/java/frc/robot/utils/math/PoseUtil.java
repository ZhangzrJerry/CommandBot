...
public class PoseUtil {
  /**
   * Represents a 2D pose with associated uncertainty (covariance matrix).
   * This is useful for tracking pose estimates with varying levels of confidence.
   */
  public static class UncertainPose2d {
    ...
    public UncertainPose2d(Pose2d pose) {
      this(pose, Matrix.eye(Nat.N3()).times(1e-9)); // Small epsilon instead of zero for numerical stability
    }
    ...
    // Variance getters
    public double getXVariance() { return covariance.get(0, 0); }
    public double getYVariance() { return covariance.get(1, 1); }
    public double getThetaVariance() { return covariance.get(2, 2); }

    // Covariance getters
    public double getXYCovariance() { return covariance.get(0, 1); }
    public double getXThetaCovariance() { return covariance.get(0, 2); }
    public double getYThetaCovariance() { return covariance.get(1, 2); }
    public double getYXCovariance() { return covariance.get(1, 0); }
    public double getThetaXCovariance() { return covariance.get(2, 0); }
    public double getThetaYCovariance() { return covariance.get(2, 1); }

    // Variance setters
    public void setXVariance(double xVariance) { covariance.set(0, 0, xVariance); }
    public void setYVariance(double yVariance) { covariance.set(1, 1, yVariance); }
    public void setThetaVariance(double thetaVariance) { covariance.set(2, 2, thetaVariance); }
    ...
    /**
     * Computes the Mahalanobis distance to another pose.
     * This accounts for the uncertainty in both poses.
     * @param other The other pose
     * @return Mahalanobis distance between the poses
     */
    public double mahalanobisDistance(UncertainPose2d other) {
      Matrix<N3, N1> diff = new Matrix<>(Nat.N3(), Nat.N1());
...
/**
 * Transforms this pose by the given uncertain transform and propagates the uncertainty.
 * This properly accounts for both the pose's uncertainty and the transform's uncertainty.
 * 
...
public UncertainPose2d transformBy(UncertainTransform2d transform) {
  ...
  // Cov_new = J_pose * Cov_pose * J_pose^T + J_transform * Cov_transform * J_transform^T
  Matrix<N3, N3> newCovariance = 
      jacobianPose.times(covariance).times(jacobianPose.transpose())
      .plus(jacobianTransform.times(transform.getCovariance()).times(jacobianTransform.transpose()));
  ...
  return new UncertainPose2d(newPose, newCovariance);
     ...
     */
    public UncertainTransform2d relativeTo(UncertainPose2d other) {
      Transform2d transform = pose.relativeTo(other.pose);
      ...
      return new UncertainTransform2d(transform, newCovariance);
    }
    ...
    public UncertainPose2d fuseWith(UncertainPose2d other) {
      ...
      Matrix<N3, N1> fusedPoseVec = fusedCov.times(invCov1.times(poseVec1).plus(invCov2.times(poseVec2)));
