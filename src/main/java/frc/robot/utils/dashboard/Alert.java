...
public class Alert {
   ...
   */
  public Alert(String alertMessage, AlertType alertType) {
   ...
   * @param alertType The severity level of the alert
   */
  public Alert(String groupName, String alertMessage, AlertType alertType) {
  ...
  private static class AlertGroup implements Sendable {
    ...
    public String[] getAlertMessages(AlertType alertType) {
      Predicate<Alert> activeFilter = (Alert x) -> x.alertType == alertType && x.isActive;
      Comparator<Alert> timeSorter =
          (Alert a1, Alert a2) -> (int) (a2.activationTime - a1.activationTime);
